 ...
 */
#ifndef OPENXCOM_GLOBEWATER_H
#define OPENXCOM_GLOBEWATER_H
 ...
 */
class GlobeWater : public InteractiveSurface
{
private:
...
public:
	...
	GlobeWater(Game *game, int cenX, int cenY, int width, int height, std::vector<double> radius, int x = 0, int y = 0);
	/// Cleans up the land.
	~GlobeWater();
	/// Sets the palette of the land.
	void setPalette(SDL_Color *colors, int firstcolor = 0, int ncolors = 256);
	/// Draws the whole land.
	void draw(double cenLon, double cenLat, Sint16 cenX, Sint16 cenY, size_t zoom, std::list<Polygon*> cacheWater, std::vector<double> radius);
	/// Draws the land of the globe (if no land textures are given).
	void drawLand(Sint16 cenX, Sint16 cenY, size_t zoom, std::vector<double> radius);
	/// Draws the ocean of the globe.
	void drawOcean(size_t zoom, std::list<Polygon*> cacheWater);
