...
XcomResourcePack::XcomResourcePack(std::vector<std::pair<std::string, ExtraSprites *> > extraSprites, std::vector<std::pair<std::string, ExtraSounds *> > extraSounds) : ResourcePack()
{
	...
	for(int i=0; i< 16; ++i )
	{
		...
		dim.end_y = 9;
		head.setDomain(dim);
		ShaderDraw<HairBleach, false>(head, ShaderScalar<Uint8>(HairBleach::Face+5));
		dim.beg_y = 9;
		dim.end_y = 10;
		head.setDomain(dim);
		ShaderDraw<HairBleach, false>(head, ShaderScalar<Uint8>(HairBleach::Face+6));
		surf->unlock();
	}
	...
	SurfaceSet *handob = _sets["HANDOB.PCK"];
	for (int i = handob->getTotalFrames() - 1; i >= 0; --i) //resize _sets["HANDOB2.PCK"] only once
	{
		Surface* surf = handob->getFrame(i);
		if(surf)
			surf->blit(_sets["HANDOB2.PCK"]->addFrame(i));
	}
